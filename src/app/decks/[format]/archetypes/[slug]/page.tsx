...
export default async function ArchetypePage({ params, searchParams }: Props) {
  ...
  return (
    <main className="page-shell deck-page">
      <Link className="deck-back-link" href={`/decks/${format}`}>← Arquetipos de {formatInfo[format].label}</Link>
      <header className="deck-intro"><div><p className="eyebrow">{formatInfo[format].label} · {group.deck_count} listas</p><h1 className="page-title">{group.name}</h1>{group.description && <p>{group.description}</p>}</div></header>
