    ...
    <main className="page-shell organization-page">
      ...
      <section className="form-card organization-settings-card">
        ...
        <form action={updateOrganization}>
          ...
          <fieldset className="format-fieldset"><legend>Formatos relacionados</legend><div className="format-options">{formats.map((format) => <label key={format}><input defaultChecked={organization.formats.includes(format)} name="formats" type="checkbox" value={format} /><span>{format}</span></label>)}</div></fieldset>
